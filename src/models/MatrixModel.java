...
public class MatrixModel extends Observable {
	...
	public void buildMatrix(String[] csv, int rows, int cols) {
		...
		csv = Arrays.copyOfRange(csv, 2, csv.length);
		...
		resultMatrix = new MatrixSearch(new Matrix(mat), new State(new Point(0,0)),new State(new Point(0,0))) ;
		setChanged();
	...
	public void setStartPosition(Point start) {
		resultMatrix.setInitialState(new State<>(start));
	}

	public void setExitPosition(Point exit) {
		resultMatrix.setGoalState(new State<>(exit));
	}
	...
	public void sendToServer(String[] output) {
		sendProblemToServer(resultMatrix.getMatrix());
		for (String o : output) {
	...
	public void getPathFromServer() {
		...
		String entrance = resultMatrix.getInitialState().getState().x + "," + resultMatrix.getInitialState().getState().y;
		String exit = resultMatrix.getGoalState().getState().x + "," + resultMatrix.getGoalState().getState().y;
